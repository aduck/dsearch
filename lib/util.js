...
// 获取将要上映电影
async function getWillPlay(){
  let $=cheerio.load(await getSource('https://movie.douban.com/coming'),{
    decodeEntities:false
  })
  let results=[]
  $('tbody tr','table.coming_list').each((i,item)=>{
    let $item=$(item)
    let $_item=$item.find('td')
    let date=$_item.eq(0).text().trim()
    let title=$_item.eq(1).text().trim()
    let category=$_item.eq(2).text().trim()
    let country=$_item.eq(3).text().trim()
    let num=$_item.eq(4).text().trim()
    results.push({
      date,
      title,
      category,
